...
export default function Calendar() {
  ...
  const renderHeader = () => {
    return (
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-4xl font-bold text-[#154A37] font-display">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => setCurrentMonth(new Date())} className="px-6 py-2 bg-[#F5F1EA] text-[#154A37] rounded-full font-bold text-sm hover:bg-[#E8E2D9] transition-colors">
            Today
          </button>
  ...
  const renderDays = () => {
    ...
    for (let i = 0; i < 7; i++) {
      days.push(
        <div key={i} className="text-center font-bold text-[#6B6B6B] text-xs uppercase tracking-widest py-4 bg-[#FDFBF7] border-b border-[#E8E2D9]">
        ...
        days.push(
          ...
          >
            <div className={`flex justify-end mb-1`}>
              <span className={`w-7 h-7 flex items-center justify-center rounded-full text-sm ${isSameDay(day, new Date()) ? 'bg-[#154A37] text-white font-bold' : 'font-medium'}`}>
                {formattedDate}
              </span>
            </div>
            <div className="space-y-1">
              ...
              {dayTasks.length > 3 && (
                <div className="text-xs text-[#6B6B6B] px-1 font-semibold">+{dayTasks.length - 3} more</div>
              )}
            </div>
          </div>
        );
